...
DEFUN_DLD (StochasticEditDist03, args, nargout,"")
{
			...
			for (int i = 0; i < n1; i++){
			   for (int j = 0; j < n2; j++){
			      if (s1[i] == s2[j]){
						...
						ExpInsCost = pe; 
						ExpMatchCost = 1-(3*pe); 
			      		Df(i+1,j+1) = Df(i+1,j+1) + Df(i,j)*ExpMatchCost;
			      }else{
			      		ExpDelCost = pe; 
						ExpInsCost = pe; 
						ExpNonMatchCost = pe; 			
			      		Df(i+1,j+1) = Df(i+1,j+1) + Df(i,j)*ExpNonMatchCost;
			      }
			      //octave_stdout << "Df\n" << Df << "\n";
			      Df(i+1,j+1) = Df(i+1,j+1) + Df(i+1,j)*ExpInsCost;
			      //octave_stdout << "Df\n" << Df << "\n";
			      Df(i+1,j+1) = Df(i+1,j+1) + Df(i,j+1)*ExpDelCost;
			   }
			}
