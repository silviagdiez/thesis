...
DEFUN_DLD (RspEditDistLog, args, nargout,"")
{
...
// Otherwise, make sure there are exactly 6 inputs, and set edit costs accordingly.
	...
	{	
		...
		//octave_stdout << "S1\n" << s1 << "\n";
		//octave_stdout << "S2\n" << s2 << "\n";
		double DelCost  = log(1.0);
		double InsCost  = log(1.0);
		double ReplCost = log(1.0);
			   ...
			   for (int j = 0; j < n2; j++){
			      if (s1[i] == s2[j]){
                     ...
                     // ExpRepl = ExpReplCost;
         			 // Repl = ReplCost;
			   		 m(0) = d;
			   		 m(1) = Df(i,j) + Repl + ExpRepl + Db(i+1,j+1);
			   		 m(2) = Df(i+1,j) + InsCost + ExpInsCost + Db(i+1,j+1);
			   		 m(3) = Df(i,j+1) + DelCost + ExpDelCost + Db(i+1,j+1);
			   		 double mx = m.max();
			   		 double logd = log(exp(d - mx) + exp(Df(i,j) + Repl + ExpRepl + Db(i+1,j+1) - mx) + 
