...
DEFUN_DLD (StochasticEditDistLog, args, nargout,"")
{
		...
		if (! error_state)
		{
			...
			for (int i = 0; i < n1; i++){
			   for (int j = 0; j < n2; j++){
			      ...
			      m(1) = Df(i,j) + ExpMatchCost;
			      m(2) = Df(i+1,j) + ExpInsCost;
			      m(3) = Df(i,j+1) + ExpDelCost;
			      double mx = m.max();
			      //Df(i+1,j+1) = Df(i,j)*ExpNonMatchCost + Df(i,j)*ExpMatchCost + Df(i+1,j)*ExpInsCost + Df(i,j+1)*ExpDelCost;
			      double logDf = log(exp(Df(i,j) + ExpNonMatchCost - mx) + exp(Df(i,j) + ExpMatchCost - mx) + exp(Df(i+1,j) + ExpInsCost - mx) + exp(Df(i,j+1) + ExpDelCost - mx)) + mx;
			      Df(i+1,j+1) = logDf;
			   }
			}
