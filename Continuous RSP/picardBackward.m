function [Uini,U1] = picardBackward(N)
...
q = MaxIter;
tol = tol;
%f = inline('-4*(x == 4)*(y == 4)','x','y'); % D = 1
%g = inline('(x == 4)*(y == 4)','x','y'); 
f = inline('-4*(abs(x-3.6364)<0.001)*(abs(y-3.6364)<0.001)','x','y'); % D = 1
g = inline('(abs(x-3.6364)<0.001)*(abs(y-3.6364)<0.001)','x','y'); 
funf = 1;
fung = 1;
